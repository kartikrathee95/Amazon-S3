...
from app.models.models import FileVersion as DBFileVersion
from app.models.models import Folder, Permission
from app.models.models import User as DBUser
from app.models.schemas import (AccessType, FileSearchRequest,
                                FileUploadRequest, FileVersion, FolderCreate,
                                PermissionCreate, RollbackRequest, Token, User,
                                UserCreate)
from app.utils.connection import (SessionLocal, create_database_if_not_exists,
                                  create_tables)
...
async def upload_file(
    file_upload: FileUploadRequest, current_user: DBUser = Depends(get_current_user)
):
    db: Session = SessionLocal()
    ...
    try:
        ...
        file_record = (
            db.query(DBFile)
            .filter(DBFile.file_name == filename, DBFile.user_id == current_user.id)
            .first()
        )
        ...
        if file_record:
            ...
            last_version = (
                db.query(DBFileVersion)
                .filter(DBFileVersion.id == file_record.id)
                .order_by(DBFileVersion.version_number.desc())
                .first()
            )
        ...
        if folder_name:
            folder = (
                db.query(Folder)
                .filter(Folder.name == folder_name, Folder.user_id == current_user.id)
                .first()
            )
            if folder is None:
                folder = Folder(
...
async def download_file(file_id: int, current_user: DBUser = Depends(get_current_user)):
    ...
    try:
        ...
        file_record = (
            db.query(DBFile)
            .filter(DBFile.id == file_id, DBFile.user_id == current_user.id)
            .first()
        )
        ...
        for file in unique_files:
            ...
            file_version = (
                ...
                .filter(
                    ...
                    DBFileVersion.version_number == current_version,
                )
                .first()
            )
            if file_version:
...
@app.post("/S3/folders")
def create_folder(
    folder: FolderCreate, current_user: DBUser = Depends(get_current_user)
):
    db: Session = SessionLocal()
    try:
...
async def list_files_and_folders(current_user: DBUser = Depends(get_current_user)):
    ...
    try:
        ...
        response = {"folders": [], "independent_files": []}
...
def share_file(file_id: int, share_request: PermissionCreate):
    ...
    try:
        ...
        permission = Permission(user_id=user.id, file_id=file.id, access_type="shared")
...
def get_file_versions(file_id: int):
    ...
    try:
        ...
        versions = (
            db.query(DBFileVersion)
            .filter(DBFileVersion.id == file_id)
            .order_by(DBFileVersion.version_number)
            .all()
        )

        if not versions:
            raise HTTPException(
                status_code=404, detail="No versions found for this file"
            )
...
async def rollback_file(
    ...
    try:
        ...
        file_version = (
            db.query(DBFileVersion)
            .filter(
                DBFileVersion.id == file_id,
                DBFileVersion.version_number == version_number,
            )
            .first()
        )
