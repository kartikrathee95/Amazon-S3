...
def test_get_profile(client, token):
    response = client.get(
        "/S3/auth/profile", headers={"Authorization": f"Bearer {token}"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_upload_file(client, token):
    ...
    response = client.post(
        "/S3/files/upload", json=file_data, headers={"Authorization": f"Bearer {token}"}
    )
    assert response.status_code == 200
...
def test_download_file(client, token, mock_db):
    ...
    response_files = client.get(
        "/S3/files", headers={"Authorization": f"Bearer {token}"}
    ).json()
    response = client.get(
        f"/S3/files/download/{response_files[0]['file_id']}",
...
def test_delete_file(client, token):
    ...
    response = client.delete(
        f"/S3/files/{response_files[0]['file_id']}",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 200
...
def test_rollback_file(client, token):
    ...
    if response_files:
        ...
        response = client.post(
            f"/S3/files/rollback",
            json={"file_id": 1001, "version_number": 1},
            headers={"Authorization": f"Bearer {token}"},
        )
...
def test_share_file(client, token):
    ...
    if response_files:
        ...
        response = client.post(
            f"/S3/share_file/{response_files[0]['file_id']}",
            json=share_data,
            headers={"Authorization": f"Bearer {token}"},
        )
        assert response.status_code == 200
